...
int main()
{
    ...
    if (wsa_err != 0)
    {
       C_LOG << "Error while determining WinSock set-up ... terminating.\m"; 
       return 1;
    }
    ...
    SOCKET base_sock {socket(c_match_addr -> ai_family, c_match_addr -> ai_socktype, 
    ...
    int connect_err {connect(base_sock, c_match_addr -> ai_addr,
                static_cast<int>(c_result_addr -> ai_addrlen))};
    if (connect_err == SOCKET_ERROR)
    {
        C_LOG << "Error in connecting to host: " << WSAGetLastError()
                << " ... terminating.\n";
        WSACleanup();
